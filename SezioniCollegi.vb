...
Public Class SezioniCollegi
    ...
    Public Sub New()
        ...
        Try
            LoadConsultazioni()
            cboConsultazioni.SelectedItem = "Elezioni Provinciali - Ballottaggio 21-22 Giugno 2009"
    ...
    Private Sub SetStyleGridSezioni()
        Try
            grid.Columns(0).Width = 120
    ...
    Private tableVotiSezioni As New EAPVoti.VotiSezioniDataTable
    ...
    Private Sub cmdDownloadXLSEuropee_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
    ...
    Private Sub cmdIndietro_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
    ...
    Private Sub cmdDownloadXLS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDownloadXLS.Click
        Try
            Dim fileName As String = "Cosenza_Scrutinio_" + Now.ToString("ddMMyyyy_hhmmss") + ".xlsx"
            Dim pathRoot As String = Gizmox.WebGUI.Forms.VWGContext.Current.HttpContext.Request.PhysicalApplicationPath + "Resources\"
    ...
    Private Sub cmdShowSezioni_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdShowSezioni.Click
        Try
            Dim consultazione As String = cboConsultazioni.SelectedItem
            If (consultazione.IndexOf("Ballottaggio") >= 0) Then
                If (Not consultazione Is Nothing) Then
                    Dim IDConsultazione As Integer = GetIDConsultazione(consultazione)
                    Dim visualizzazione As String = cboVisualizzazione.SelectedItem
                    If (Not visualizzazione Is Nothing) Then
                        ...
                        If (Not collegio Is Nothing) Then
                            ...
                            If (consultazione.IndexOf("Ballottaggio") >= 0) Then
                                BuildHeaderColumn(IDConsultazione)
                                ShowDatiBallottaggio(IDConsultazione, IDCollegio, collegio)
                ...
                End If
                grid.DataSource = tableVotiSezioni

                SetStyleGridSezioni()
            Else
                Gizmox.WebGUI.Forms.MessageBox.Show("Questa funzione è abilitata solo per il Ballottaggio Provinciali 2009.", "Informazione", Gizmox.WebGUI.Forms.MessageBoxButtons.OK, Gizmox.WebGUI.Forms.MessageBoxIcon.Information)
    ...
    Private Sub BuildHeaderColumn(ByVal IDConsultazione As Integer)
        Try
            Dim adapter As New EAPVotiTableAdapters.soraldo_ele_gruppoTableAdapter
            Dim gruppi As DataTable = adapter.GetDataByIDConsultazione(IDConsultazione)
            For Each gruppo As EAPVoti.soraldo_ele_gruppoRow In gruppi.Rows
                Dim nomeGruppo As String = gruppo.descrizione
                Dim column As New DataColumn
                column.Caption = nomeGruppo
                column.ColumnName = nomeGruppo
                column.DataType = GetType(String)
                column.AllowDBNull = True

                tableVotiSezioni.Columns.Add(column)
            Next
        Catch ex As Exception
            UtilityContainer.ErrorLog(ex)
    ...
    Private Sub ShowDatiBallottaggio(ByVal IDConsultazione As Integer, ByVal IDCollegio As Integer, ByVal collegio As String)
        Try
            tableVotiSezioni.Rows.Clear()

            Dim adapterGruppi As New EAPVotiTableAdapters.soraldo_ele_gruppoTableAdapter
            Dim gruppi As DataTable = adapterGruppi.GetDataByIDConsultazione(IDConsultazione)
            Dim adapter As New EAPVotiTableAdapters.soraldo_ele_sezioniTableAdapter
            Dim sezioni As DataTable = Nothing
            If (IDCollegio > 0) Then
                sezioni = adapter.GetDataByIDConsultazioneIDCollegio(IDConsultazione, IDCollegio)
            Else
                sezioni = adapter.GetDataByIDConsultazione(IDConsultazione)
            End If

            For Each sezione As EAPVoti.soraldo_ele_sezioniRow In sezioni.Rows
                Dim numeroSezione As Integer = sezione.num_sez
                Dim IDSezione As Integer = sezione.id_sez
                Dim row As EAPVoti.VotiSezioniRow = tableVotiSezioni.NewVotiSezioniRow

                row.Collegio = collegio
                row.Sezione = numeroSezione
                For Each gruppo As EAPVoti.soraldo_ele_gruppoRow In gruppi.Rows
                    Dim nomeGruppo As String = gruppo.descrizione
                    Dim IDGruppo As Integer = gruppo.id_gruppo
                    Dim votiValidi As Integer = GetVotiSezione(IDConsultazione, IDSezione, IDGruppo)
                    row(nomeGruppo) = votiValidi
                Next

                tableVotiSezioni.AddVotiSezioniRow(row)
    ...
    Private Function GetVotiSezione(ByVal IDConsultazione As Integer, ByVal IDSezione As Integer, ByVal IDGruppo As Integer) As Integer
        Try
            Dim adapter As New EAPVotiTableAdapters.soraldo_ele_voti_gruppoTableAdapter
            Dim voti As DataTable = adapter.GetDataByIDConsultazioneIDGruppoIDSezione(IDConsultazione, IDGruppo, IDSezione)
            Dim votiSezione As Integer = 0
            If (voti.Rows.Count >= 1) Then
                Dim voto As EAPVoti.soraldo_ele_voti_gruppoRow = voti.Rows(0)
                votiSezione = voto.voti
