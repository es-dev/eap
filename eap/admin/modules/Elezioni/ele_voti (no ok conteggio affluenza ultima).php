...
function stato_rilevazione($id_sez,$solosede){
global $id_cons, $prefix, $dbi, $id_cons_gen,$status,$genere,$votol;
$ops=0;
$status=0;
	if($ops==0){
#controllo congruenza affluenze
		$res = mysql_query("SELECT orario,data FROM ".$prefix."_ele_rilaff where id_cons_gen=$id_cons_gen order by data desc,orario desc", $dbi);
		if($res)
		while (list($ora,$giorno) = mysql_fetch_row($res)){
			$query="SELECT sum(voti_complessivi) FROM ".$prefix."_ele_voti_parziale as t1 where t1.id_sez='$id_sez' and t1.orario='$ora' and t1.data='$giorno'";
			$res2 = mysql_query("$query", $dbi);
			list($voti)=mysql_fetch_row($res2);
			if($voti) {
				if(isset($ora)){
					$query="SELECT count(voti_complessivi) FROM ".$prefix."_ele_voti_parziale as t1 where t1.id_cons='$id_cons' and t1.orario='$ora' and t1.data='$giorno'";
					$res3 = mysql_query("$query", $dbi);
					list($numscru)=mysql_fetch_row($res3);
					$res3= mysql_query("SELECT count(num_sez) FROM ".$prefix."_ele_sezioni where id_cons='$id_cons'", $dbi);
					list($numtot) = mysql_fetch_row($res3);
					$res3= mysql_query("SELECT maschi+autorizzati_m,femmine+autorizzati_f FROM ".$prefix."_ele_sezioni where id_sez='$id_sez'", $dbi);
					list($iscrittim,$iscrittif) = mysql_fetch_row($res3);
					$iscritti=$iscrittim+$iscrittif;
					
					$query="SELECT voti_complessivi, voti_uomini, voti_donne FROM ".$prefix."_ele_voti_parziale as t1 where t1.id_sez='$id_sez' and t1.orario='$ora' and t1.data='$giorno'";
					$res3 = mysql_query("$query", $dbi);
					list($voti, $uomini, $donne)=mysql_fetch_row($res3);
#					if($voti>0) {#echo $query;
						if ((($uomini+$donne)==0 and ($voti)<=$iscritti) or ((($uomini+$donne)==$voti) and ($uomini<=$iscrittim) and ($donne<=$iscrittif))){
							#if($numscru<$numtot and $numscru>0)
								{$status=1;$ops=-1;break;}#
#							else
#								{$status=0;$ops=-1;break;}
						}else {return 1;}
#					} else return 0;
				}
			} else {return -1;}
		...
		# return 0;
		#controllo congruenza voti espressi
		$res2= mysql_query("SELECT sum(validi) FROM ".$prefix."_ele_sezioni where id_cons='$id_cons'", $dbi);
		if($res2){
		list($validi) = mysql_fetch_row($res2);
			if($validi) {
					$status=0;
					$query="SELECT validi,nulli,bianchi,contestati,voti_nulli,solo_gruppo,validi_lista,contestati_lista,voti_nulli_lista FROM ".$prefix."_ele_sezioni as t1 where t1.id_sez='$id_sez'";
					$res4 = mysql_query("$query", $dbi);
					list($validi, $nulli, $bianchi,$contestati,$votinulli,$sologruppo,$validilista,$contestatilista,$votinullilista)=mysql_fetch_row($res4);
					if($validi>0) {
		...
		#controllo congruenza voti di gruppo
		$res3= mysql_query("SELECT sum(voti) FROM ".$prefix."_ele_voti_gruppo where id_cons='$id_cons'", $dbi);
		if($res3){$status=0;$ops=-4;
			list($votig) = mysql_fetch_row($res3);
			if($votig) {  
					$status=0;
					$query="SELECT sum(voti) FROM ".$prefix."_ele_voti_gruppo where id_sez='$id_sez'";
					$res4 = mysql_query("$query", $dbi);
					list($votigs)=mysql_fetch_row($res4);
					if($votigs>0) {
						if (($votigs)==$validi)
							{$status=1;$ops=-3;}
						else return 4;
		...
		#controllo congruenza voti di lista
		$res3= mysql_query("SELECT sum(voti) FROM ".$prefix."_ele_voti_lista where id_cons='$id_cons'", $dbi);
		if($res3 and ($genere>1 and !$votol)){$status=0;
			list($votil) = mysql_fetch_row($res3);
			if($votil) {
						
					$query="SELECT sum(voti) FROM ".$prefix."_ele_voti_lista where id_sez='$id_sez'";
					$res4 = mysql_query("$query", $dbi);
					list($votils)=mysql_fetch_row($res4);
					if($votils>0) {
						if (($votils+$sologruppo+$contestatilista+$votinullilista)==($validi+$votinulli+$contestati))
							{$status=1;$ops=-3;}
						else {return 3;}
					} #else return 0;
			}
...
//	list ($perms)=mysql_fetch_row($ressup);
	$solosede='';
	if ($perms<32) {
		$res = mysql_query("SELECT id_sede FROM ".$prefix."_ele_operatori where aid='$aid' and id_cons=$id_cons", $dbi);
		if ($res) {
			list($solouna)=mysql_fetch_row($res);
			if ($solouna>0) $solosede="and id_sede='$solouna'";
			}
		}
		$res = mysql_query("SELECT num_sez, id_sez ,id_sede FROM ".$prefix."_ele_sezioni where id_cons='$id_cons' $solosede order by num_sez", $dbi);
		$max = mysql_num_rows($res);
		//if ($max>=64){
		$i=0;
		while(list($sez_num, $sez_id ,$sede_id) = mysql_fetch_row($res)) {
			$ops='';
			if(!$id_sez) {$id_sez=$sez_id; $id_sede=$sede_id;}
			$i++;
			...
			$sezstat=0; #die($ops);
			global $status;
			$sezstat=stato_rilevazione($sez_id,$solosede);
			$cursez='';
			$bgsez="";
#			if ($sezstat==(-1)) $bgsez="#99CC33"; #verde
			if ($status==1 and $sezstat<0) { $ops=abs($sezstat);$bgsez="#99CC33";} #verde
			elseif ($sezstat<0) {$ops=abs($sezstat);}
			elseif ($sezstat>0) {
				$bgsez="#FF3300";#rosso;
				if ($sezstat) $ops=$sezstat; else $ops=1;
				}
			else{$bgsez="";}
			if(!$ops) $ops=1;
			$res2= mysql_query("SELECT num_sez FROM ".$prefix."_ele_sezioni where id_sez='$id_sez'", $dbi);
			list($numero_sez) = mysql_fetch_row($res2);
			if ($sez_num==$numero_sez) {$cursez= "style=\" border-color: YELLOW; border-width: 5px;
\"";}#$bgsez="#FFFF00";}
...
function voti($id_cons,$do,$id_circ,$id_sede,$ops,$ov,$mv,$gv,$msv,$av,$id_lista) {
   	...
   	echo "<table border=\"0\" width=\"770\" align=\"left\"><tr>";
...
*/
	...
	//************************************
		...
		//$bgcolor1="#b0b0b0";
   		echo "<hr><table border=\"0\" width=\"60%\"  align=\"center\"><tr>";
...
$bgcolor2=$_SESSION['bgcolor2'];
	...
	echo "<table border=\"0\" width=\"90%\" align=\"center\"><tr bgcolor=\"$bgcolor1\" align=\"center\">";
