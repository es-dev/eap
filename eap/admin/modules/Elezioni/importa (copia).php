...
$tabs=array($prefix."_ele_gruppo",$prefix."_ele_lista",$prefix."_ele_candidati");
$x=0;$k=0;
$scarto=0;
$conta=array();
/*    while( $currentLine <= $cntFile ){
	...
	$conta[$x]=0;
	$conf=$tabs[$x];
#	die(substr($arrFile[$currentLine],1,-2)."==$conf and $currentLine <= $cntFile");
...
#    while( $currentLine  <= $cntFile ){
		...
		$conf=$tabs[$x];
if($k==0) {while (substr($arrFile[$currentLine],1,-2)!=$conf and $currentLine <= $cntFile) $currentLine++; $k++;}
#		die("qui arriva $currentLine  <= $cntFile  $conf=".$tabs[$x]);
#		if ($tab==$conf){
			$currentLine++;
			while($currentLine <= $cntFile ){
				$appo=substr($arrFile[$currentLine],1,-2);
#				if ($appo==$prefix."_ele_circoscrizione"){ break;}
				if ($appo==$tabs[($x+1)]){ $x++;$conf=$tabs[$x];$currentLine++; continue;}
				$test=explode(':',$arrFile[$currentLine]); if(!is_array($test)) {die("errore di import<br>");}
#				$valori='';
				foreach($test as $key=>$val) #echo "$key : $campo ($conf==$prefix._ele_gruppo)"; die();
						...
						elseif ($appo==$prefix."_ele_circoscrizione")
							{$fine=0;break;}
			$currentLine++;
				$z++;
			}
...
#unset($valori);
$sigruppo=count($ar_gruppo);die("sigru:$sigruppo");
...
foreach ($ar_gruppo as $rigagruppo){
	foreach($rigagruppo as $key=>$campo) {
		if ($key==0) echo "$valori=\"'$idcns',";
		elseif ($key==1) $valori.= "null";
		else $valori.= ",'$campo'";
		if ($key==2) $numgruppo= $campo;
#
}	
		die("$res_gruppo = mysql_query(\"insert into ".$prefix."_ele_gruppo values($valori)\" ,$dbi");
		$resnew = mysql_query("select id_gruppo from ".$prefix."_ele_gruppo where num_gruppo='$numgruppo' and id_cons='$idcns'" ,$dbi);
		list ($newidg) = mysql_fetch_row($resnew);
*/
########continuare da qui 
...
if (isset($errore))
	if ($errore) die( _MEX_IMPORT_FAILED);
	else  echo _MEX_IMPORT_OK;
echo"</td></tr></table>";
include("footer.php");
